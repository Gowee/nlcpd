...
def main():
    ...
    def f(book):
        ...
        if re.search(
            r"中華.+法$|六法|法案|(法令|章程|規則|細則|法規|紀錄|報告書?)(分類)?([匯選簡合彙續][編集]|[概輯譯提]要|[輯一]覽表?|集|全書|補錄|[總簡年]表|年[報鑑]|手冊|(報告)?書?|[匯選簡合彙續]報)?$|工作報告|[特專彙匯]刊$|(大會|選舉|判例)彙刊$|令$|參[議政][會院]|大?會議?[紀記]?錄$|決議案$|聯合會|省憲法|[時新公月旬]報",
            book["name"],
        ) and not re.search(
